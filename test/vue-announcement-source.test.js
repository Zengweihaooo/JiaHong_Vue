...
test("Vue announcement components use unread dots and mark details as read", async () => {
  const [homeDashboard, latestAnnouncementCard, dialogs, store, uiStyles, legacyStyles] = await Promise.all([
    readFile(new URL("../src/components/home/HomeDashboard.vue", import.meta.url), "utf8"),
    readFile(new URL("../../JiaHong_UI/src/components/LatestAnnouncementCard/LatestAnnouncementCard.vue", import.meta.url), "utf8"),
  ...
  assert.match(homeDashboard, /import \{[^}]*LatestAnnouncementCard[^}]*\} from "@jiahong\/ui";/s);
  assert.match(homeDashboard, /<LatestAnnouncementCard\s+:announcement="store\.latestAnnouncement"\s+@detail="openAnnouncementDetail"\s+@history="store\.announcementListVisible = true"/);
  assert.match(homeDashboard, /store\.markAnnouncementRead\(announcement\.id\)/);
  assert.doesNotMatch(homeDashboard, /NoticeCard/);

  assert.match(latestAnnouncementCard, /name: 'LatestAnnouncementCard'/);
  assert.match(latestAnnouncementCard, /announcement__unread-dot/);
  ...
  assert.match(dialogs, /AnnouncementContent/);
  ...
  assert.match(uiStyles, /\.announcement-highlight\s*\{/);
  assert.match(uiStyles, /\.announcement-content__line\s*\{/);
